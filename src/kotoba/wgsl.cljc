(ns kotoba.wgsl
  "WGSL as data — 'hiccup for shaders'. An EDN AST compiles to a WGSL source string, so a game's
   ...
   source feeds kotoba.webgpu and fixtures without hand-copying shader text. `.cljc`: the same
   compiler runs on shadow-cljs (browser) and bb/JVM.
   ...
   Arithmetic/calls come from the shared kotoba.expr core; WGSL only customises the surface — idents are
...
(def ^:private ctors {:vec2 "vec2<f32>" :vec3 "vec3<f32>" :vec4 "vec4<f32>"
                      ...
                      ;; short type spellings (WGSL aliases) — used by render shader generators
